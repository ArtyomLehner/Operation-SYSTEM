...
// Çàïèñü ðåçóëüòàòà â îáùèé ôàéë ñ ìüþòåêñîì (òàêàÿ æå, êàê âûøå)
...
int main(int argc, char *argv[]) {
    if (argc == 2) {
        // Ðåæèì äî÷åðíåãî ïðîöåññà: îäèí àðãóìåíò – ÷èñëî n
        int n = atoi(argv[1]);
    ...
    // Óäàëÿåì ñòàðûé ôàéë ðåçóëüòàòîâ
    ...
    for (int i = 0; i < numChildren; i++) {
        ...
        CloseHandle(pi.hThread);  // ïîòîê íå íóæåí
        hProcesses[i] = pi.hProcess;
    }

    // Æä¸ì çàâåðøåíèÿ âñåõ äî÷åðíèõ ïðîöåññîâ
    WaitForMultipleObjects(numChildren, hProcesses, TRUE, INFINITE);

    // Çàêðûâàåì äåñêðèïòîðû
    for (int i = 0; i < numChildren; i++) {
